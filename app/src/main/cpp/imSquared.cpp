...
static void
replace(std::string &target, std::string const &match, std::string const &replace_string) {
...
void imSquared::loadDatabase() {
    ...
    for (Figures::iterator figure = m_figures.begin(); figure != m_figures.end(); ++figure) {
        ...
        DPrintf("color: %.2f %.2f %.2f %.2f\n", figure->color.red, figure->color.green,
                figure->color.blue, figure->color.alpha);
        DPrintf("sound: %s\n", figure->sound.c_str());
...
void imSquared::initialize(Configuration const &configuration) {
    ...
    m_configuration.squareWidth =
            ((float) m_configuration.squareWidth / (float) m_configuration.screenWidth) * 2.0;

    m_configuration.squareHeight =
            (float) m_configuration.screenHeight / (float) (m_configuration.rows - 1);
    m_configuration.squareHeight =
            ((float) m_configuration.squareHeight / (float) m_configuration.screenHeight) * 2.0;
...
void imSquared::update() {
    ...
    if (m_translation > m_configuration.squareHeight) {
        ...
        for (int currentColumn = 0; currentColumn != m_configuration.columns; ++currentColumn) {
            for (int currentLine = (m_configuration.rows - 1); currentLine >= 0; --currentLine) {
                if (currentLine > 0) {
                    Point vertices[4];
                    memcpy(vertices, m_matrix[currentLine][currentColumn].vertices,
                           sizeof(vertices));
                    memcpy(&(m_matrix[currentLine][currentColumn]),
                           &(m_matrix[currentLine - 1][currentColumn]), sizeof(SquareElement));
                    memcpy(m_matrix[currentLine][currentColumn].vertices, vertices,
                           sizeof(vertices));
                } else {
                    ...
                    if (m_configuration.texture_enabled) {
                        ...
                        m_matrix[currentLine][currentColumn].color.red = (0.5f +
                                                                          sin((float) currentTime) *
                                                                          0.3f);
    ...
    m_speed += updateDelta *
               m_levels[m_currentLevel].speed_increment_per_second[m_configuration.hardness];
    ...
    if ((!m_levels.empty() && (m_figureGeneration > m_levels[m_currentLevel].total_figures) &&
         m_figuresOnBoard.empty())) {
        m_ended = true;
...
void imSquared::processLeavingFigures() {
    ...
    //
    for (SquareElements::iterator i = m_matrix.rbegin()->begin();
         i != m_matrix.rbegin()->end(); ++i) {
        if (i->state == Marked) {
            m_multiplier = 1;
        }
    }
...
void imSquared::processFigures() {
    ...
    if (m_currentFigure == -1) {
        ...
        if (m_figureGeneration < m_levels[m_currentLevel].total_figures) {
            ...
            m_currentFigureOffset =
                    rand() % (m_configuration.columns - m_figures[m_currentFigure].width + 1);
    ...
    if (m_currentFigureLine < m_figures[m_currentFigure].height) {
        for (int x = 0; x < m_figures[m_currentFigure].width; x++) {
            if (m_figures[m_currentFigure].lines[m_currentFigureLine][x] == 'X') {
                ...
                m_matrix[0][m_currentFigureOffset +
                            x].markedColor = m_figures[m_currentFigure].color;
...
void imSquared::render() {
...

    {
        ...
        m_font.Bind(m_configuration.screenWidth, m_configuration.screenHeight);
        m_font.Print(message, 5, m_configuration.screenHeight - 5 - (m_font.GetHeight()));
        m_font.Unbind();
    }
...
void imSquared::ShowMainMenu() {
    ...
    m_menu.Show();
    DPrintf("imSquared::ShowMainMenu() finished");
}
