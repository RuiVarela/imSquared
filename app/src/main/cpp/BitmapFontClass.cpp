...
bool CBitmapFont::Load(const char *fname) {
    ...
    switch (RenderStyle) {
        case BFG_RS_ALPHA:
            glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, ImgX, ImgY, 0, GL_LUMINANCE,
...
void CBitmapFont::Print(char ch, float x, float y) {
    ...
    for (int i = start; i != 5; ++i) {
        ...
        vertex[1 * 2 + 0] = offset_x + x + ((float) CellX) * BaseScaler * CurrentScaler;
        vertex[1 * 2 + 1] = offset_y + y;
        coords[1 * 2 + 0] = U1;
        coords[1 * 2 + 1] = V1;

        vertex[2 * 2 + 0] = offset_x + x;
        vertex[2 * 2 + 1] = offset_y + y + ((float) CellY) * BaseScaler * CurrentScaler;
        coords[2 * 2 + 0] = U;
        coords[2 * 2 + 1] = V;

        vertex[3 * 2 + 0] = offset_x + x + ((float) CellX) * BaseScaler * CurrentScaler;
        vertex[3 * 2 + 1] = offset_y + y + ((float) CellY) * BaseScaler * CurrentScaler;
        coords[3 * 2 + 0] = U1;
        coords[3 * 2 + 1] = V;
...
void CBitmapFont::Print(std::string const &text, int x, int y) {
    for (int i = 0; i != text.size(); ++i) {
        Print(text[i], x, y);
        x += Width[(int)text[i]] * BaseScaler * CurrentScaler;
    }
}
